...
add_action( 'show_user_profile', 'ramal_cargo_setor' );
add_action( 'edit_user_profile', 'ramal_cargo_setor' );

function ramal_cargo_setor( $user ) { ?>
    <h3><?php _e("Ramal, Cargo e Setor", "blank"); ?></h3>

    <table class="form-table">
        <tr>
            <th><label for="ramal"><?php _e("Ramal"); ?></label></th>
            <td>
                <input type="number" min="1" name="ramal" id="ramal" value="<?php echo esc_attr( get_the_author_meta( 'ramal', $user->ID ) ); ?>" class="regular-text" /><br />
                <span class="description"><?php _e("Informe o número do seu ramal"); ?></span>
            </td>
        </tr>
        <tr>
            <th><label for="setor"><?php _e("Setor"); ?></label></th>
            <td>
                <input type="text" name="setor" id="setor" value="<?php echo esc_attr( get_the_author_meta( 'setor', $user->ID ) ); ?>" class="regular-text" /><br />
                <span class="description"><?php _e("Informe qual setor você trabalha."); ?></span>
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <span class="description"><?php _e("Informe seu cargo aqui."); ?></span>
            </td>
        </tr>
    </table>
<?php }
add_action( 'personal_options_update', 'save_ramal_cargo_setor' );
add_action( 'edit_user_profile_update', 'save_ramal_cargo_setor' );

function save_ramal_cargo_setor( $user_id ) {
    if ( !current_user_can( 'edit_user', $user_id ) ) { 
        return false; 
    }
    update_user_meta( $user_id, 'ramal', $_POST['ramal'] );
